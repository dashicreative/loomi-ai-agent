...
class WebSearchTool:
    ...
    async def search(
        ...
        query: str,
        result_count: int = 30,
        search_strategy: str = "mixed",  # "priority_only", "mixed", "broad"
        ...
        """
        Execute recipe search with flexible parameters.
        
        Args:
            query: Search query (will auto-append "recipe" if not present)
            result_count: Number of results to return (10-100)
            search_strategy: 
    ...
    async def _execute_search(
        ...
        if self.serpapi_key:
            ...
            raise ValueError("No search provider configured. Need either SerpAPI or Google Custom Search credentials.")
