...
from discovery_tools import DiscoverySearchTool, DiscoveryClassificationTool, DiscoveryParsingTool, DiscoveryListTool, DiscoveryComposer
from strict_tools import StrictSearchTool, StrictClassificationTool, StrictParsingTool, StrictListTool
from shared_tools import AgentPerformanceScorer, SessionPerformanceTracker
...
class HybridAgentDeps:
    ...
    # Discovery and Strict tool instances (initialized in post_init)
    discovery_composer: DiscoveryComposer = field(init=False)
    
    # Strict tools (current architecture - unchanged for now)
    search_tool: DiscoverySearchTool = field(init=False)
    classification_tool: DiscoveryClassificationTool = field(init=False)
    recipe_parsing_tool: DiscoveryParsingTool = field(init=False)
    list_parsing_tool: DiscoveryListTool = field(init=False)
    ...
    def __post_init__(self):
        """Initialize both discovery and strict tool instances."""
        # Discovery tools (new fast pipeline)
        self.discovery_composer = DiscoveryComposer(self)
        
        # Strict tools (current architecture - unchanged for now)
        self.search_tool = DiscoverySearchTool(self.serpapi_key, self.google_key, self.google_cx)
        self.classification_tool = DiscoveryClassificationTool(self.openai_key)
        self.recipe_parsing_tool = DiscoveryParsingTool(self.openai_key)
        self.list_parsing_tool = DiscoveryListTool(self.openai_key)
        
        # Performance tracking
        self.scorer = AgentPerformanceScorer()
        self.session_tracker = SessionPerformanceTracker()
        
        # Enhanced session state for discovery vs strict mode deduplication
        self.session_shown_urls_discovery = set()
        self.session_shown_urls_strict = set()


# Load system prompt
def load_hybrid_system_prompt() -> str:
    """Load the discovery + strict system prompt from file."""
    prompt_path = Path(__file__).parent.parent / "hybrid_system_prompt.txt"
    try:
        return prompt_path.read_text()
    except FileNotFoundError:
        return """You are Loomi, an expert recipe discovery agent with discovery and strict tools.
        Use discover_recipes for exploration, search_and_parse_recipes for precision."""
...
async def discover_recipes(
    ...
    """
    DISCOVERY TOOL: Fast, broad recipe exploration optimized for browsing.
    
    Search engine approach: leverage Google's relevance + minimal post-processing.
    Target: ≤8 seconds, 10-15+ varied results for exploration and browsing.
    
    Use for: broad queries, exploration, minimal constraints, variety seeking.
    """
    tool_start = time.time()
    
    print("=" * 80)
    print("🔍 DISCOVERY MODE SELECTED - FAST EXPLORATION PIPELINE")
    print("=" * 80)
    print(f"📝 Query: '{query}'")
    print(f"🎯 Target: {target_recipe_count} varied recipes for browsing")
    print(f"⚡ Speed Goal: ≤8 seconds")
    print("-" * 80)
    
    if not ctx.deps.query_start_time:
        ctx.deps.query_start_time = time.time()
    
    # Use discovery-specific session exclusion
    result = await ctx.deps.discovery_composer.discover_recipes(
        query=query,
        target_recipe_count=target_recipe_count,
        search_url_count=search_url_count,
        exclude_urls=ctx.deps.session_shown_urls_discovery
    )
    
    # Update discovery session URLs
    for recipe_summary in result.get("recipe_summaries", []):
        if recipe_summary.get("url"):
            ctx.deps.session_shown_urls_discovery.add(recipe_summary["url"])
    
    tool_elapsed = time.time() - tool_start
    total_elapsed = time.time() - ctx.deps.query_start_time
    
    print("=" * 80)
    print("✅ DISCOVERY MODE COMPLETE")
    print("=" * 80)
    print(f"📊 Results: {len(result.get('recipe_summaries', []))} recipes ready for browsing")
    print(f"⏱️  Total Time: {total_elapsed:.1f}s | Tool Time: {tool_elapsed:.1f}s")
    print(f"🚀 Performance: {'✅ FAST' if total_elapsed <= 8 else '⚠️ SLOW'} (target ≤8s)")
    print("=" * 80)
...
async def search_and_parse_recipes(
    ...
    print("=" * 80)
    print("🎯 STRICT MODE SELECTED - PRECISION CONSTRAINT PIPELINE")
    print("=" * 80)
    print(f"📝 Query: '{query}'")
    print(f"🎯 Subject: {subject or 'Not specified'}")
    print(f"⏱️  Target: 4-8 precise recipes with constraint verification")
    print(f"🎯 Speed Goal: ≤15 seconds")
    print("-" * 80)
    ...
    print(f"🔒 CONSTRAINTS: dietary={dietary_constraints}, allergies={combined_allergies}")
    print(f"🔒 NUTRITION: {nutrition_requirements}, time={time_constraints}")
    print(f"🔍 SEARCH: {url_count} URLs, {search_strategy} strategy")
    if user_profile_allergies and ignore_profile_allergies:
        print(f"⚠️ SAFETY OVERRIDE: Profile allergies ignored: {user_profile_allergies}")
    print("-" * 80)
    ...
    try:
        # STEP 1: Search for URLs (strict mode with separate session exclusion)
        search_result = await ctx.deps.search_tool.search(
            query=query,
            result_count=url_count,
            search_strategy=search_strategy,
            exclude_urls=ctx.deps.session_shown_urls_strict
        )
        ...
        # Update strict session state with found URLs
        for recipe in parsed_recipes:
            if recipe.get("source_url"):
                ctx.deps.session_shown_urls_strict.add(recipe["source_url"])
        ...
        print("=" * 80)
        print("✅ STRICT MODE COMPLETE")
        print("=" * 80)
        print(f"📊 Results: {len(parsed_recipes)} precise recipes, {len(list_urls)} backlog lists")
        print(f"⏱️  Total Time: {elapsed:.1f}s | Tool Time: {tool_elapsed:.1f}s")
        print(f"🎯 Performance: {'✅ FAST' if elapsed <= 15 else '⚠️ SLOW'} (target ≤15s)")
        print(f"📈 Quality: {result.get('quality_assessment', 'unknown').upper()}")
        print("=" * 80)
