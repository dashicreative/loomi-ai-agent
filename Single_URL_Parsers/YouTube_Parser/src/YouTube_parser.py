...
Extracts and processes recipes from YouTube videos using:
1. Apify YouTube Audio Downloader (for audio extraction)
2. Deepgram Nova-2 (for transcription)
3. Shared VerticalVideoProcessor (for recipe parsing steps 5-10)

Complete pipeline: YouTube URL → Audio → Transcript → Recipe JSON
"""
...
class YouTubeTranscriber:
    """
    Extract and process recipes from YouTube videos.

    Pipeline:
    1. Extract YouTube data using Apify YouTube Audio Downloader
    2. Download audio from YouTube CDN
    ...
    def __init__(self, google_api_key: str = None):
        ...
        self.temp_dir = tempfile.mkdtemp(prefix="youtube_transcriber_")
        self.temp_files = []

    def extract_with_apify(self, youtube_url: str) -> Dict[str, Any]:
        """
        Extract YouTube audio data using Apify's YouTube Audio Downloader.

        Args:
            youtube_url: YouTube video URL

        Returns:
            Dictionary with audio_url, description, thumbnail_url, and metadata
        """
        actor_id = "alpha-scraper~youtube-audio-downloader"
        ...
        try:
            print(f"🚀 Step 1: Extracting YouTube data with Apify...")
                            ...
                            if default_dataset_id:
                                ...
                                if dataset_response.status_code == 200:
                                    ...
                                    if items and len(items) > 0:
                                        ...
                                        # Extract audio URL from nested selected_downloadable_audio_link object
                                        selected_audio = item.get("selected_downloadable_audio_link", {})
                                        ...
                                        return {
                                            ...
                                            "uploader": "",  # YouTube doesn't provide uploader in this actor
                                            "upload_date": "",  # YouTube doesn't provide upload date in this actor
                                            "duration": item.get("duration", ""),
                                            "view_count": "",  # YouTube doesn't provide this
                                            "like_count": "",
                                            "comment_count": "",
    ...
    def download_audio_from_url(self, audio_url: str) -> str:
        """
        Download audio from YouTube CDN URL.
        ...
        """
        print(f"🔽 Step 2: Downloading audio from YouTube CDN...")

        audio_path = os.path.join(self.temp_dir, "youtube_audio.mp3")
    ...
    def parse_youtube_recipe_to_json(self, youtube_url: str) -> str:
        """
        Complete pipeline: Extract YouTube video and return structured recipe JSON.
        ...
        try:
            # Step 1: Extract YouTube data with Apify
            step_start = time.time()
            apify_data = self.extract_with_apify(youtube_url)
