#!/usr/bin/env python3
"""
Test CLI for complete Facebook recipe parsing (Steps 1-10).

Tests full pipeline:
1-4: Facebook-specific extraction
...
def main():
    """
    Test the complete Facebook recipe parsing pipeline.
    """
    print("=" * 70)
    print("📘 Facebook Recipe JSON Parser Test (Complete Pipeline)")
    print("=" * 70)
    ...
    # Get Facebook URL from user
    facebook_url = input("Enter Facebook URL: ").strip()
    ...
    if "facebook.com" not in facebook_url:
        print("❌ Please provide a valid Facebook URL.")
        return

    print(f"\n🔄 Processing: {facebook_url}")
    ...
    try:
        ...
        transcriber = FacebookTranscriber()

        print("\n🚀 Running Facebook recipe parsing pipeline...\n")

        # Parse Facebook recipe to JSON
        recipe_json = transcriber.parse_facebook_recipe_to_json(facebook_url)
