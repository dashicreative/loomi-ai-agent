...
Extracts and processes recipes from TikTok videos using:
1. Apify TikTok Audio Downloader (for audio extraction)
2. Deepgram Nova-2 (for transcription)
3. Shared VerticalVideoProcessor (for recipe parsing steps 5-10)

Complete pipeline: TikTok URL → Audio → Transcript → Recipe JSON
"""
...
class TikTokTranscriber:
    """
    Extract and process recipes from TikTok videos.

    Pipeline:
    1. Extract TikTok data using Apify TikTok Audio Downloader
    2. Download audio from TikTok CDN
    ...
    def __init__(self, google_api_key: str = None):
        ...
        self.temp_dir = tempfile.mkdtemp(prefix="tiktok_transcriber_")
        self.temp_files = []

    def extract_with_apify(self, tiktok_url: str) -> Dict[str, Any]:
        """
        Extract TikTok audio data using Apify's TikTok Audio Downloader.

        Args:
            tiktok_url: TikTok video URL

        Returns:
            Dictionary with audioUrl, caption, thumbnailUrl, and metadata
        """
        ...
        try:
            print(f"🚀 Step 1: Extracting TikTok data with Apify...")
    ...
    def download_audio_from_url(self, audio_url: str) -> str:
        ...
        """
        print(f"🔽 Step 2: Downloading audio from TikTok CDN...")

        audio_path = os.path.join(self.temp_dir, "tiktok_audio.mp3")
    ...
    def parse_tiktok_recipe_to_json(self, tiktok_url: str) -> str:
        """
        Complete pipeline: Extract TikTok video and return structured recipe JSON.
        ...
        try:
            # Step 1: Extract TikTok data with Apify
            step_start = time.time()
            apify_data = self.extract_with_apify(tiktok_url)
