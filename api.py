...
class SilentPushRequest(BaseModel):
    ...
    deviceToken: str
    userId: str
    jobId: str  # Client-generated UUID for tracking
...
# Generate unique instance ID to track multiple Railway instances
INSTANCE_ID = str(uuid.uuid4())[:8]
print(f"\n{'='*60}")
print(f"🚀 API INSTANCE STARTED - ID: {INSTANCE_ID}")
print(f"{'='*60}\n")
...
# Initialize parsers (once at startup)
...
_lock_creation_lock = asyncio.Lock()  # Global lock to prevent race condition when creating user locks
...
async def process_user_queue(user_id: str):
    ...
    """
    print(f"🔄 [QUEUE] [INSTANCE:{INSTANCE_ID}] Starting queue processor for user {user_id[:8]}...")
    ...
    try:
        while True:
            ...
            async with global_semaphore:
                ...
                # Update Firebase status: queued → processing
                if firebase_db:
                    try:
...
async def process_recipe_background(url: str, device_token: str, user_id: str, job_id: str):
    ...
    try:
        ...
        # Update Firebase status: processing → completed
        ...
        # Update Firebase status: processing → failed
        if firebase_db:
            try:
                print(f"📝 [FIREBASE] Updating status to 'failed' for job {job_id}")
                ...
                print(f"   [FIREBASE] Status updated to 'failed' with error message")
            except Exception as firebase_error:
                print(f"⚠️  [FIREBASE] Failed to update status to failed: {str(firebase_error)}")
...
@app.post("/parse-tiktok-recipe", response_model=ParseResponse)
async def parse_tiktok_recipe(request: URLRequest):
    """
    Parse TikTok video into structured recipe JSON.

    Input: {"url": "https://www.tiktok.com/@user/video/123456789"}
    ...
    try:
        print(f"🎵 TikTok Parse Request: {request.url}")

        # Validate TikTok URL
        if "tiktok.com" not in request.url.lower():
            raise HTTPException(status_code=400, detail="URL must be a TikTok video")
        ...
        # Parse TikTok recipe (not async - runs in thread pool automatically)
        print("🚀 Starting TikTok parser...")
        recipe_json = tiktok_parser.parse_tiktok_recipe_to_json(request.url)

        elapsed_seconds = time.time() - start_time
        print(f"✅ TikTok parsing completed successfully in {elapsed_seconds:.2f}s")
        ...
        # Log full traceback for debugging
        print(f"❌ TikTok Parse Error after {elapsed_seconds:.2f}s: {error_message}")
        print("📍 Full traceback:")
        print(traceback.format_exc())
        ...
        if "private video" in error_message.lower() or "private account" in error_message.lower():
            status_code = 403
            user_message = "This TikTok video is private or from a private account"
        elif "deleted" in error_message.lower() or "not available" in error_message.lower() or "not found" in error_message.lower():
            status_code = 404
            user_message = "TikTok video not found or has been deleted"
        elif "rate limit" in error_message.lower() or "too many requests" in error_message.lower():
            status_code = 429
            user_message = "Rate limit exceeded, please try again later"
        elif "timeout" in error_message.lower():
...
@app.post("/parse-facebook-recipe", response_model=ParseResponse)
async def parse_facebook_recipe(request: URLRequest):
    """
    Parse Facebook video into structured recipe JSON.

    Input: {"url": "https://www.facebook.com/watch/?v=123456789"}
    Output: {"success": true, "recipe_json": "...", "elapsed_seconds": 12.5}
    """
    import traceback
    import time
    ...
    try:
        print(f"📘 Facebook Parse Request: {request.url}")

        # Validate Facebook URL
        if "facebook.com" not in request.url.lower():
            raise HTTPException(status_code=400, detail="URL must be a Facebook video")
        ...
        # Parse Facebook recipe (not async - runs in thread pool automatically)
        print("🚀 Starting Facebook parser...")
        recipe_json = facebook_parser.parse_facebook_recipe_to_json(request.url)

        elapsed_seconds = time.time() - start_time
        print(f"✅ Facebook parsing completed successfully in {elapsed_seconds:.2f}s")
        ...
        # Log full traceback for debugging
        print(f"❌ Facebook Parse Error after {elapsed_seconds:.2f}s: {error_message}")
        print("📍 Full traceback:")
        print(traceback.format_exc())
        ...
        if "private video" in error_message.lower() or "private account" in error_message.lower():
            status_code = 403
            user_message = "This Facebook video is private or from a private account"
        elif "deleted" in error_message.lower() or "not available" in error_message.lower() or "not found" in error_message.lower():
            status_code = 404
...
async def queue_recipe_silent_push(request: SilentPushRequest):
    ...
    try:
        ...
        # Use client-provided job ID (generated by ShareViewController)
        job_id = request.jobId

        print(f"📋 [QUEUE] [INSTANCE:{INSTANCE_ID}] Queuing recipe processing job {job_id}")
        print(f"   [QUEUE] Using client-provided jobId")
        ...
        # Use global lock to safely create user-specific lock (prevents race on lock creation)
        async with _lock_creation_lock:
            if request.userId not in user_queue_locks:
                user_queue_locks[request.userId] = asyncio.Lock()
                print(f"   [QUEUE] Created lock for user {request.userId[:8]}")

        # Now use user-specific lock to check/create processor
        async with user_queue_locks[request.userId]:
            if request.userId not in user_queue_processors or user_queue_processors[request.userId].done():
                print(f"   [QUEUE] [INSTANCE:{INSTANCE_ID}] Starting NEW queue processor for user {request.userId[:8]}")
                processor_task = asyncio.create_task(process_user_queue(request.userId))
                user_queue_processors[request.userId] = processor_task
            else:
                print(f"   [QUEUE] [INSTANCE:{INSTANCE_ID}] Queue processor ALREADY RUNNING for user {request.userId[:8]}")
